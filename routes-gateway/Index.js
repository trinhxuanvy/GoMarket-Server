...
router.post('/api/v1/Auth/Shipper/:type', async (req, res) => {
  switch (req.params.type.toLowerCase()) {
    case 'register':
      axios
        .post(`http://localhost:8000/api/v1/Auth/Shipper/register`, req.body)
        .then((response) => {
          console.log(response.data);
          res.send(response.data);
        })
    ...
    default:
      axios
        .post(`http://localhost:8000/api/v1/Auth/Shipper/login`, req.body)
        .then((response) => {
          res.send(response.data);
        })
        .catch(() => {
...
router.get('/api/v1/Shipper/byId', async (req, res, next) => {
  axios
    .get(`http://localhost:8000/api/v1/Shipper/byId`, {
      headers: req.headers,
    })
    .then((response) => {
      res.send(response.data);
...
router.post('/api/v1/user/Shipper/update', async (req, res, next) => {
  ...
  console.log(req.body[0]);
  axios
    .post(`http://localhost:8000/api/v1/user/Shipper/update`, req.body[0], {
      headers: {
        'content-type': 'application/json',
        authorization: req.headers?.authorization,
      },
